...
export class Searchbar extends Component<{}, DefaultProps> {
    ...
    componentDidMount(): void {
        fetch(`${process.env.NEXT_PUBLIC_SUPABASE_URL}/users`)
        .then(response => response.json())
        .then(users => this.setState({ users: users }));

        fetch(`${process.env.NEXT_PUBLIC_SUPABASE_URL}/posts`)
        .then(response => response.json())
        .then(posts => this.setState({ posts: posts }));

        // fetch(`${process.env.NEXT_PUBLIC_SUPABASE_URL}/community`)
        // .then(response => response.json())
        // .then(communities => this.setState({ communities: communities }));
        
        // fetch(`${process.env.NEXT_PUBLIC_SUPABASE_URL}/gltfs`)
        // .then(response => response.json())
    ...
    render() {
        ...
        const filteredPosts = posts.length > 0 ? posts.filter(post =>
            post.postContent?.toLowerCase().includes(searchField.toLowerCase())) : [];
